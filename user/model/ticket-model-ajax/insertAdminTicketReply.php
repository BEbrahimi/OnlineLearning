...
if (empty($msg)){
    ...
    $admin_email = sanitize($_POST['admin_ticket_email']);
    $ticket_num = sanitize($_POST['ticket_num']);
    $status = 1;

    $sql=("INSERT `$tbl_user_ticket` SET `ticket_num`=?,`ticket_subject`=?,`ticket_body`=?,`priority`=?,`user_email`=?,`admin_email`=?,`status`=?");
    ...
    $result->bindValue(6,$admin_email);
